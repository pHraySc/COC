...
public class EChartCombinImpl implements IChart {
    public EChartCombinImpl() {
    }
    ...
    public String createChart(ChartModel chartModel, List<Map<String, Object>> data) {
        ...
        for(int optionStr = 0; optionStr < legendData.length; ++optionStr) {
            ...
            for(int bar = 0; bar < data.size(); ++bar) {
                Map guideline = (Map)data.get(bar);
            ...
            Object[] lineData;
            Guideline var32;
            if(optionStr == legendData.length - 1) {
                Line var30 = new Line();
                var32 = (Guideline)guidelineMap.get(name);
                ((Line)var30.name((String)name)).yAxisIndex(Integer.valueOf(var32.getValueAxisIndex()));
                ...
                lineData = values.toArray(new Object[values.size()]);
                var30.data(lineData);
                var25.series(new Series[]{var30});
            } else {
                Bar var31 = new Bar();
                var32 = (Guideline)guidelineMap.get(name);
                ((Bar)var31.name((String)name)).yAxisIndex(Integer.valueOf(var32.getValueAxisIndex()));
                ((Label)var31.itemStyle().normal().label().show(Boolean.valueOf(true))).position(Position.top);
                lineData = values.toArray(new Object[values.size()]);
                var31.data(lineData);
                var25.series(new Series[]{var31});
            }
        }
