...
import com.ailk.biapp.ci.util.ziputil.zip.EncryptDeflater;
...
public class EncryptZipOutput extends EncryptDeflater implements ZipConstants {
    ...
    public void putNextEntry(EncryptZipEntry e) throws IOException {
        this.ensureOpen();
        if(this.entry != null) {
            this.closeEntry();
        }
        ...
        if(!this.finished) {
            ...
            if(this.entries.size() < 1) {
                ...
                while(e.hasMoreElements()) {
                    this.writeCEN((EncryptZipEntry)e.nextElement());
                }
    ...
    private void writeLOC(EncryptZipEntry e) throws IOException {
    ...
    private void writeEXT(EncryptZipEntry e) throws IOException {
    ...
    private void writeCEN(EncryptZipEntry e) throws IOException {
        ...
        this.writeInt(e.csize);
        this.writeInt(e.size);
        byte[] nameBytes = this.getBytes(e.name);
        this.writeShort(nameBytes.length);
        this.writeShort(e.extra != null?e.extra.length:0);
        byte[] commentBytes;
        if(e.comment != null) {
    ...
    private byte[] getBytes(String s) throws UnsupportedEncodingException {
        Object bytes = null;
        byte[] bytes1;
        if("UTF-8".equalsIgnoreCase(this.getEncoding())) {
            bytes1 = getUTF8Bytes(s);
        } else {
            bytes1 = s.getBytes(this.getEncoding());
        }
