...
<chart caption="${dim:toName(DIM_OP_LOG_TYPE,ciLogStatAnalysisModel.opTypeId) }"  yAxisName="次数︵次︶" palette="2" baseFontSize="12"  showValues="0" 
		...
		canvasBorderColor="#cccccc" formatNumberScale="0" showShadow="0" anchorBgColor="#6BCF39"  
		anchorBorderColor="F8F8F8" anchorRadius="5" anchorBorderThickness="3" lineThickness="2" 
		 slantLabels="1" lineColor="#6BCF39" plotGradientColor="" showPlotBorder="0">
<categories >
<category label="" />
<c:forEach items="${multDeptOptypesChartList}" var="verifyMm">
<category label="${verifyMm.thirdDeptName}" />
</c:forEach>
<category label="" />
</categories>
...
<dataset seriesName="${dim:toName(DIM_OP_LOG_TYPE,ciLogStatAnalysisModel.parentId) }">
	<set />
		<c:forEach items="${multDeptOptypesChartList}" var="verifyMm">
			<set value="${verifyMm.opTimes1}" label="${verifyMm.thirdDeptName}" />
		</c:forEach>
	<set />
</dataset>
<c:choose>
	...
	<c:otherwise>
		...
		<set />
		<c:forEach items="${multDeptOptypesChartList}" var="verifyMm">
		<set value="${verifyMm.opTimes}" label="${verifyMm.thirdDeptName}"  />
		</c:forEach>
		<set />
		</dataset>
	</c:otherwise>
