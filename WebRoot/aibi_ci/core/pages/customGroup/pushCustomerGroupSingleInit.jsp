...
<html>
<head>
<c:set var="province" value="<%=province%>"></c:set>
...
<form id="pushForm" method="post" action="" enctype="multipart/form-data">
					...
					<table width="100%" class="showTable  new_tag_table" cellpadding="0" cellspacing="0">
						<tr>
							...
							<td>
								<div class="new_tag_selectBox">
									<input type="hidden" id="customGroupIdHidden" name="customGroupInfo.customGroupId" value="${customGroupInfo.customGroupId}" />
									<span id="selectCycle">
										<c:if test="${isPush == 1}">
										...
										<c:if test="${customGroupInfo.updateCycle == 3 && customGroupInfo.createTypeId != 12}">
											<c:if test="${isPush == 1}">
										...
										<c:if test="${customGroupInfo.updateCycle == 2 && customGroupInfo.createTypeId != 12}">
											<c:if test="${isPush == 1}">
						...
						<tr>
							...
							<td>
								...
								<c:if test="${customGroupInfo.updateCycle == 3}">
								...
								<c:if test="${customGroupInfo.updateCycle == 2}">
		 ...
		 <input type="hidden" id="isPush" value="${isPush}" />
		 <input type="hidden" id="pushCycle" value="${customGroupInfo.updateCycle}" />
...
var isPush = '${isPush}';
var sysIds = [];
var pushCycle = '${customGroupInfo.updateCycle}';
	...
	$.ajax({
        ...
        success	: function (data) {
        	...
        	if(duplicateNum != 0 && duplicateNum != null) {
        		window.parent.$.fn.weakTip({"tipTxt":"用户群包含重复记录不能进行推送设置！"});
        		if(url) {
