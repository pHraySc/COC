				...
				easyloader.load('datagrid', function(){ //加载指定模块 
					$("#groupGridList").datagrid({ 
					    ...
					    columns:[[
							...
							{field:'customNum',title: '用户群规模',align: 'right',width: 80,formatter:function(value,rec){
							...
							{field:'dataStatusStr',title: '数据状态',align: 'left',width: 69,
								formatter:function(value,rec){
									...
									if(rec.createTypeId == 1 && dataStatusVar == '<%=ServiceConstants.CUSTOM_DATA_STATUS_FAILED%>'){
										...
										*/
										html="<p class='query-item-total-txt-my-custom'>"+rec.dataStatusStr
											+"</p><a title='重新生成' href='javascript:void(0);' onclick='regenCustomer(\""+groupId+"\",\""+rec.createTypeId+"\",\""+rec.listTableName+"\")' class='query-item-total-icon'/>";
									}else{
							...
							{field:'opt',title:'操作',width:85,align:'center',
								formatter:function(value,rec){  
								var btn = '<a class="editcls" id="showSqlInfo" style="text-decoration: underline;" onclick="showSqlInfo(\''+rec.listTableName+'\')" href="javascript:void(0)">查看执行详情</a>';  
								return btn;  
								}  
							}  
					  	]],
	...
	<div>
	 <div class="tableDottedWrap">
            	...
            	<table>
                    	<tbody>
                    	<tr class="firstTr">
                        	<td colspan="4" align="left" title="用户群名称" >&nbsp;${ciCustomGroupInfo.customGroupName}
                        	   <c:if test= "${ciCustomGroupInfo.isAlarm == true}">
                        <!--icon 根据实际情况显示 -->
						<div class="tag_main_icon">
							<span><img style="cursor:pointer;width: 15px;"  title="用户群告警" src="${ctx}/aibi_ci/assets/themes/default/images/warning.png" class="main_icon_alarm"/></span>
                            <!-- 告警弹出层start -->
                            ...
                            <div class="slideDown relative_help">
                                <dl>
                                    ...
                                    <dd>
                                                                                                           环比预警：将用户群当前统计周期的用户数与上一周期用户数进行比较，针对差值进行预警<br/>
                                                                                                           环比增长量 = 用户群当前统计周期用户数 - 用户群上一周期用户数
                                    </dd>
                                </dl>
                            </div>
                            <!-- 告警弹出层end -->
                    	...
                    	<tr>
                          ...
                          <th width="25%" title="地市" >${ciCustomGroupInfo.createCityName }</th>
                          <th width="25%"></th>
                        </tr>
                    	<tr class="solid">
