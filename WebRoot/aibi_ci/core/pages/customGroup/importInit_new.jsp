...
<div id="importClientDialog">
	<div class="dialogHeaderTitle">
		<p class="dialogTitleText"> 导入用户群</p>
	...
	<form id="saveForm" method="post" action="${ctx}/core/ciCustomerManagerAction!uploadCustomGroupFile.ai2do" enctype="multipart/form-data">
	    <ol class="clearfix">
		    <li> 
				<label class="fleft labFmt110 star">用户群名称：</label>
			  	<input type="text" name="ciCustomGroupInfo.customGroupName" id="customGroupName" 
			  	class="fleft inputFmt305" value="" maxlength="35" />
			  	<!-- 提示 -->
			...
			<li>  
			     <label class="fleft labFmt110">用户群描述：</label>
			...
			<li style="display:none;">
			    ...
			    <div class="fleft">
			       <a  class="fleft importFileModelBtn " href="${ctx}/core/ciCustomerManagerAction!downImportTempletFile.ai2do" title="">下载导入文件模板</a>
	...
	<div class="btnActiveBox">
		<a href="javascript:void(0);" id="saveBtn" onclick="importInit();" class="ensureBtnLong">保存用户群</a>
	</div>
</div>
...
<script type="text/javascript">
	...
	function submitList(){
		if(validateForm()){
			//加上遮罩层
			parent.showLoading('正在导入用户群文件，请耐心等候...');
			var actionUrl = $.ctx+'/core/ciCustomerManagerAction!isNameExist.ai2do';
	...
	function customGroupAttValueRepeat() {
	    ...
	    if(!flag) {
	    	$("#anameTip").empty();
			$("#anameTip").show().append("用户群属性名称重复！");
	    } else {
	    	$("#anameTip").hide();
	    }
	    return flag;
		...
		if(validateForm2()){
			...
			$.post(actionUrl,$("#importByTableForm").serialize(), function(result){
				if(result){
					...
					var submitUrl =  $.ctx+'/core/ciCustomerManagerAction!createCustomByImportTable.ai2do';
					...
					$.post(submitUrl,params, function(result){
						if(result.success){
							$.fn.weakTip({"tipTxt":result.msg,"backFn":function(){
								...
								//成功后把弹出层关掉
								parent.$("#dialog-form").dialog("close");
							}});
						}else{alert("error");
	...
	function validateForm2(){
		...
		if(customGroupName == ""){
			...
			$("#cnameTip2").empty();
			$("#cnameTip2").show().append("请输入用户群名称");
			return false;
		}else{
