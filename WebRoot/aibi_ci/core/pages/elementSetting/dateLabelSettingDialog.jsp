...
<head>
	...
	$(function(){
		var startTime = '${param["ciLabelRule.startTime"]}';
		var endTime = '${param["ciLabelRule.endTime"]}';
		var dateLeftClosed = '${param["ciLabelRule.leftZoneSign"]}';
		var dateRightClosed = '${param["ciLabelRule.rightZoneSign"]}';
		var queryWay = '${param["ciLabelRule.queryWay"]}';//区分是日期范围还是精确日期
		var exactValueDate = '${param["ciLabelRule.exactValue"]}'; //精确日期 yyyy,mm,dd 没有选的也填充为-1
		var isNeedOffset = '${param["ciLabelRule.isNeedOffset"]}';
		if(isNeedOffset == 1) {
	...
	function submitSetDate(){
		if(validateForm()){
			...
			target.attr("startTime",startTime);
			target.attr("endTime",endTime);
			target.attr("leftZoneSign",leftZoneSign);
			target.attr("rightZoneSign",rightZoneSign);
			target.attr("exactValue",exactValueDate);
			target.attr("queryWay",queryWay);
			target.attr("isNeedOffset",isNeedOffset);
			...
			parent.ShoppingCart.iframeCloseDialog("#dateLabelSetting");
		}
	}
	</script>
</head>
