...
shownames="1" showvalues="0" rotateYAxisName="0" useRoundEdges="0" legendBorderAlpha="80" bgColor="ffffff" showBorder="0" baseFont="Microsoft YaHei" outCnvBaseFont="Microsoft YaHei">
  ...
  <categories>
  	<c:if test="${!empty labelFormTrendList}">
		<c:forEach items="${labelFormTrendList}" var="brandForm" begin="1" end="1">
			<c:forEach items="${brandForm.labelFormList}" var="brandFormTrend">
  	...
  	<c:if test="${!empty labelFormTrendList}">
		<c:forEach items="${labelFormTrendList}" var="brandForm">
