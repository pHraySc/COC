...
<chart caption="${caption}" showPercentageValues='1' showShadow="0" showPlotBorder="0" canvasBorderColor="CCCCCC" plotFillRatio="100" 
...
shownames="1" showvalues="0"  useRoundEdges="0" legendBorderAlpha="0" bgColor="ffffff" showBorder="0" baseFont="Microsoft YaHei" outCnvBaseFont="Microsoft YaHei">
  ...
  <categories>
  	<c:if test="${!empty customGroupTrendList}">
		<c:forEach items="${customGroupTrendList}" var="cityForm" begin="1" end="1">
			<c:forEach items="${cityForm.customGroupFormList}" var="cityFormTrend">
  	...
  	<c:if test="${!empty customGroupTrendList}">
		<c:forEach items="${customGroupTrendList}" var="cityForm">
