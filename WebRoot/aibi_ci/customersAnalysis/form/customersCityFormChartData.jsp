...
shownames="1" showvalues="1" animation="1" legendBorderAlpha="0" borderColor="FFFFFF" plotBorderColor="FFFFFF" bgColor="ffffff" showBorder="0" baseFont="Microsoft YaHei" outCnvBaseFont="Microsoft YaHei">
	 <category label='总用户数<fmt:formatNumber type="number" value="${ciCustomGroupForm.customNum}"  pattern="###,###,###" />' Color='FFFFFF' link='javascript:getCityFormHisChart();' hoverText="查看趋势">
		<c:forEach items="${customGroupFormList}" var="cityForm">
			<c:if test="${cityForm.customNum != 0}">
				<category color="${cityForm.color}" label='<fmt:formatNumber type="number" value="${(cityForm.customNum/ciCustomGroupForm.customNum)*100}"  pattern="###,###,###.##" />%' value="${cityForm.customNum}"  link="javascript:getCityTrendChartByCityId('${cityForm.cityId}');" hoverText='点击查看${cityForm.cityName} <fmt:formatNumber type="number" value="${(cityForm.customNum/ciCustomGroupForm.customNum)*100}"  pattern="###,###,###.##" />% 近六个月趋势'/>
			</c:if>
			<c:if test="${cityForm.customNum == 0 || ciCustomGroupForm.customNum == 0}">
				<category label='' value="${cityForm.customNum}"  link="javascript:getSubcityFormChart('${cityForm.cityId}');" hoverText='${cityForm.cityName} <fmt:formatNumber type="number" value="${(cityForm.customNum/ciCustomGroupForm.customNum)*100}"  pattern="###,###,###.##" />%'/>
			</c:if>
		</c:forEach>
    </category>
