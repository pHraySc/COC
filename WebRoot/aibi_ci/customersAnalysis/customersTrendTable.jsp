...
$(document).ready( function() {
	...
	pagetable();
	$(document).on("click",function (event) {$("#pushSelectList").hide(); $("a.push_link").removeClass("push_link_on");}); 
	$("a.push_link").click(function(){
		var offset = $(this).offset();
		$("#pushSelectList").css({"top":  (offset.top + 20)+ "px", "left" : (offset.left-75) + "px"}).show();
		/*var tar=$(this).siblings(".form-field-new");
		if($(this).hasClass("push_link_on")){
			tar.hide();
			return;
		}else{
			var toffset=$(this).offset();
			tar.css({top:toffset.top+18,left:toffset.left-32});
			tar.show();
				...
				<td>
					<%if(("true").equals(customersFileDown)){ %>
						...
						<%}else{%>
							...
							<a class="icon_button_oper" style="line-height:16px;" href="$.ctx + '/ci/customersFileAction!downCustomersFile.ai2do?listTableName=${po.listTableName}" id="downFile" style="line-height:16px;">
							下载</a></c:if>
						<%}%>
					<%}%>
					<div class="form-field-new"  id="pushSelectList">
						<ul>
							<c:forEach items="${ciSysInfoList }" var="ciSysInfo">
								<li><a href="javascript:pushCustomer('${param["customGroupId"]}','${po.dataDate}','${ciSysInfo.sysId }');">${ciSysInfo.sysName }</a></li> 
							</c:forEach>
						</ul>
					</div>
				</td>
