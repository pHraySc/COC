...
<html>
	<head>
		...
		function saveUserRelLabel(){
			...
			var param = 'ciCustomRelModel.customGroupId='+customGroupId;
				...
				param += '&ciCustomRelModel.mainCustomUserNum='+mainCustomUserNum;
				param += '&ciCustomRelModel.relLabelIds='+relLabelIds;
				param += '&ciCustomRelModel.overlapUserNumStr='+overlapUserNumStr;
			var chart_url = escape($.ctx+'/ci/customersRelAnalysisAction!saveUserRelLabel.ai2do?'+param);
		...
		//将主标签与关联标签的关系保存为客户群
		function toSaveCustom(customGroupId,relLabelId,overlapUserNum) {
			$('#customGroupName').val('');
			$('#customGroupDesc').text('');
			$('#customGroupId').val(customGroupId);
		...
		function saveCustomer(){
			...
			var action_url = $.ctx+"/ci/customersRelAnalysisAction!saveRelLabelAsCustomGroup.ai2do";
			var param = '?ciCustomRelModel.customGroupId='+customGroupId;
			param += '&ciCustomRelModel.dataDate='+dataDate;
			param += '&ciCustomRelModel.relLabelId='+relLabelId;
			param += '&ciCustomRelModel.listTableName='+listTableName;
			param += '&ciCustomGroupInfo.dataDate='+dataDate;
			param += '&ciCustomGroupInfo.customGroupName='+encodeURIComponent(encodeURIComponent(customGroupName));
			param += '&ciCustomGroupInfo.customGroupDesc='+encodeURIComponent(encodeURIComponent(customGroupDesc));
			param += '&ciCustomGroupInfo.productAutoMacthFlag='+productAutoMacthFlag;
			param += '&ciCustomGroupInfo.customNum='+overlapUserNum;
			param += '&ciCustomGroupInfo.sceneId='+sname;
			action_url = action_url + param;
			$.ajax({
				url: action_url,
				type: "POST",
				async: false,
	...
	<body>
		<div class="main_con_box">
			<!--标题行开始 -->
			<div align="right" >
			    数据时间：${dateFmt:dateFormat(ciCustomRelModel.dataDate)} &nbsp;&nbsp;
			...
			<div id="createDialog" style="display:none;">
					<div class="dialog_table">
							...
							<tr>
								...
								<td>
									<div id="select1" class="new_tag_selectBox">
										<input name="" type="text" id="customGroupName" value="" maxlength="35" class="new_tag_input" onFocus="focusName(this.id);" onBlur="blurName(this.id);" style="width:302px;*width:306px;cursor:text"/>
									</div>
									<!-- 提示 -->
						...
						<input id="relLabelId" name="ciCustomRelModel.relLabelId" type="hidden" value=""/>
		                <input id="customGroupId" name="ciCustomRelModel.customGroupId" type="hidden" value="${ciCustomRelModel.customGroupId}"/>
                        <input id="customName" type="hidden" value="${ciCustomRelModel.customGroupName}" />
                        <input id="mainCustomUserNum" type="hidden" value="${ciCustomRelModel.mainCustomUserNum}"/>
                        <input id="dataDate" name="ciCustomGroupInfo.dataDate" type="hidden" value="${ciCustomRelModel.dataDate}"/>
                        <input id="listTableName" name="ciCustomRelModel.listTableName" type="hidden" value="${ciCustomRelModel.listTableName}"/>
		                <input id="relLabelIds" type="hidden" value="${ciCustomRelModel.relLabelIds}" />
		                <input id="overlapUserNumStr" value="${ciCustomRelModel.overlapUserNumStr}" type="hidden"/>
		                <input id="overlapUserNum" name="ciCustomGroupInfo.customNum" type="hidden"/>
