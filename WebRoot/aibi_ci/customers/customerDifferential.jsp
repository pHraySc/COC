...
function showCount(){
	$("#tag_analyse_dropable").find(".fright").each(function(index){
		...
		var actionUrl =  "${ctx}/ci/customersManagerAction!countIntersectOfCustomersAndLabel.ai2do";
		...
		$.post(actionUrl, $("#baseForm").serialize()+"&"+param, function(result){
				if(result.success){
					...
					$("#console").append("客户群微分：(" +result.count+")"+result.sql+"<br/>");
					
				}else{
					parent.showAlert("标签在当前时间没有数据","failed");
					$(obj).find("font").empty().append();
					//$(obj).empty().append('  ').parent().parent().append('<input type="checkbox" onclick="checkButton(this);"/>');
...
function changeNum(month){
	...
	var actionUrl =  "${ctx}/ci/customersManagerAction!countIntersectOfCustomersAndLabel.ai2do?1=1";
	var obj = this;
...
function different(id,obj){
	...
	_html += '<input type="hidden" name="ciLabelRuleList['+sortNum+'].sortNum" value="1"/>';
...
function getCustomGroupNames(){
	...
	param += "&ciCustomGroupInfo.updateCycle=1&ciCustomGroupInfo.customGroupName="+encodeURIComponent(encodeURIComponent($.trim($("#showDesc").text().substr(0,14))));
				...
				<table class="commonTable mainTag_show">
					<tr>
						...
						<td width="150" class="dateline_userNum">
							<font class="green_num bold">${ciCustomListInfo.customNum}</font>
							<p>用户数
	<%--					 		<img src="${ctx}/aibi_ci/assets/themes/default/images/icon_help.png" title="${ciCustomGroupInfo.customGroupName}"/> --%>
							</p>
							<form id="baseForm">
								<input type="hidden" id="isLastLabel" name="isLastLabel" value="0" />
								<input type="hidden" name="ciCustomListInfo.dataDate" value="${ciCustomListInfo.dataDate}"/>
								<input type="hidden" name="ciCustomListInfo.customGroupId" value="${ciCustomGroupInfo.customGroupId}"/>
								<input type="hidden" name="ciCustomGroupInfo.dataDate" value="${ciCustomListInfo.dataDate}"/>
								<input type="hidden" name="ciCustomGroupInfo.startDateStr" value="<fmt:formatDate value="${ciCustomGroupInfo.startDate }" pattern="yyyy-MM-dd"/>"/>
								<input type="hidden" name="ciCustomGroupInfo.endDateStr" value="<fmt:formatDate value="${ciCustomGroupInfo.endDate }" pattern="yyyy-MM-dd"/>"/>
								
								
								<input type="hidden" name="ciCustomSourceRelList[0].calcuElement" value="${ciCustomListInfo.listTableName}"/>
								<input type="hidden" name="ciCustomSourceRelList[0].sortNum" value="0"/>
								<input type="hidden" name="ciCustomSourceRelList[0].elementType" value="1"/>
								<input type="hidden" name="ciCustomSourceRelList[0].text" value="${ciCustomGroupInfo.customGroupName}"/>
								
								<br/>
								<input type="hidden" name="ciCustomSourceRelList[1].calcuElement" value="AND"/>
								<input type="hidden" name="ciCustomSourceRelList[1].sortNum" value="0"/>
								<input type="hidden" name="ciCustomSourceRelList[1].elementType" value="0"/>
							</form>
						</td>
					</tr>
				</table>
		...
		<dl class="analyse_right_dl">
			<dt>
				<label style="width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; word-break: keep-all;">主客户群(<font style="font-weight: bold;">${ciCustomGroupInfo.customGroupName}</font>)与标签的关联分析展示</label>&nbsp;<label id="showDesc" style="width: 900px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; word-break: keep-all;">${ciCustomGroupInfo.customGroupName}</label>
			</dt>
