...
%>
	...
	//修改客户群回写客户群 策略
	window.onload = function(){callBackData();}
	...
	$(function(){
		if (getCookieByName("${version_flag}") == "false") {
			$(".J_custom_showname").html("客户群规模");
		}else{
			$(".J_custom_showname").html("用户群规模");
		}
		if (getCookieByName("${version_flag}"=="false")){
			$(".J_SEARCH_CUSTOM_SHOWNAME").html();
		}
		//计算规则展示的位置
		...
		//最小化功能按钮
		$('.calculateCenterGoBack').click(function() {
			var preUrl = $('#calculateCenterPreUrl').val();
			if(!preUrl) {
				if(getCookieByName("${version_flag}") == 'true') {//新版
					preUrl = '/core/ciMainAction!findMarketMain.ai2do?indexLinkType=3';
				} else {//旧版
					preUrl = '/ci/ciIndexAction!labelIndex.ai2do?refreshType=2';
				}
				window.location.href = $.ctx + preUrl;
				return;
			}
		...
		//规则预览
		$('#showShopCartRuleCalculate').COClive('click', function() {
	...
	function submitForExplore(){
		...
		$.post(actionUrl, $("#labelForm").serialize(), function(result){
			if (result.success) {
				...
				$("#customerNum").empty().append("<label>用户数:</label><span>" + num + "</span>");
			} else {
	...
	function validateSql2(){
		...
		if(validateEnumCount() == false){
			showAlert("配置的条件中值超过"+_EnumCount+"个，不支持数据探索，可以保存客户群后查看用户数！","failed");
			flag = false;
			return flag;
		}
	...
	function openEdit(){
		...
		$("#editFrame").attr("src", ifmUrl).load(function(){resetTemplateForm();});
			...
			if(updateCycle){
				...
				if(updateCycle == updateCycleM){
					if(choiceDateM > dataDate  ){
						flag = false;
						commonUtil.create_alert_dialog("effectDateTip2", {
							txt 	: "当前选择的日期数据未生成！",
							type 	: "failed", 
							width 	: 500,
							height 	: 200
...
<div class="mainBody" id="calcCenterBody" >
	<div class="labelComputCtrls">
        ...
        <div class="fleft labelDate ">
        	&nbsp;&nbsp;日<input name="dayDataDate" type="text" id="dayDataDate" value="${newLabelDayFormat}" 
        		readonly class="datepicker_input" 
        		onclick="WdatePicker({onpicked:dayDatePicked, ychanged:changeEndDate,Mchanged:changeEndDate, dateFmt:'yyyy-MM-dd',maxDate:'${newLabelDayFormat}',minDate:'#F{$dp.$DV(\'${newLabelDayFormat}\',{d:-2});}'})"/>
        </div>
        <% } else {%>
        ...
        <div class="fleft ">
	        ...
	        <div class="fleft formQuerySelBox">
	      	  <div class="selTxt J_calculate_tactic_val">保守数据策略</div>
	      	  	 ...
	      	  	 <dl>
	      	  	 	<dd>
	      	  	 		<c:forEach items="${dimListTacticsList}" var="dimListTactics" varStatus="st">
	      	  	 			<a href="javascript:void(0);" value="${dimListTactics.tacticsId }" id="tacticsId_${dimListTactics.tacticsId }">${dimListTactics.tacticsName }</a>
	      	  	 		</c:forEach>
	      	  	 	</dd>
	      	  	 </dl>
	      	  </div>
		...
		<% } %>
		<% } %>
		<div class="fright  op-center-box coc-cart-content">
			<a class="fright market-navigation-icon market-cart-goto-list center-go-back calculateCenterGoBack" title="返回"></a>
			<span class="fright market-navigation-icon market-cart-tag-del center-del-all J_show_cart_box_calculate" title="清空" onclick="clearLabels()"></span>
		</div>
		<div class="fright separate">&nbsp;</div>
        <ul class="fright controlAll">
            <li><input class="hiddenCheckbox" type="checkbox" /><div class="queryWay">全部(且)</div></li>
            <li><input class="hiddenCheckbox" type="checkbox" /><div class="queryWay">全部(或)</div></li>
            <li><input class="hiddenCheckbox" type="checkbox" /><div class="queryWay">全部(剔除)</div></li>
        </ul>
         <div class="fright separate">&nbsp;</div>
         <div id="parenthesis" class="fright parenthesis">
    ...
    <div id="calculateContain" class="labelComput">
    	<jsp:include page="calculateBody.jsp" />
    </div><!--labelComput end -->
</div><!--mainBody end -->
...
<div id="tagSaveActionBox" class="mainBody tagSaveActionBox " > 
	<div class="arrowBox hidden"> 
		<div id="tagCalcDetail" class="arrowUp">折叠</div>
	</div>
	<div class="labelsCTBtm maxHeight hidden" id="labelsCTBtm ">
        <c:forEach var="result" items="${sessionModelList}">
			...
			<c:if test="${result.elementType == 5}">
            	<c:out value="客户群：${result.customOrLabelName}"></c:out><c:if test="${not empty result.attrVal}"><c:out value="[已选清单：${result.attrVal}]"></c:out>
            	</c:if>
			</c:if>
		</c:forEach>
	</div>
	<div class="tagSaveBtnBox">
	   	...
	   	<!-- <a class="cus-rule-op fright" href="javascript:void(0);" id="tagCalcSaveBtn">保存客户群</a> -->
	   	<button class="fright market-cart-button" 
						type="button" id="tagCalcSaveBtn">保存客户群</button>
		...
		<a class="cus-rule-op fright " href="javascript:void(0);" id="tagCalcExplore">客户群规模</a>
		<!-- <input type="button" value="上一步" id="calculateAddLable" /> -->
		<a class="cus-rule-op fright" href="javascript:void(0);" id="showShopCartRuleCalculate">规则预览</a>
	...
	<c:forEach var="ciGroupAttrRel" items="${ciGroupAttrRelList}" varStatus="st">
	 	<li>
	 		<div class="clientSelectItem" 
				...
				<c:if test="${ciGroupAttrRel.attrSource == 3}">${ciGroupAttrRel.customName}-</c:if>${ciGroupAttrRel.attrColName}</span>
...
<form id="labelForm" method="post" action="${ctx}/ci/customersManagerAction!saveCustomByRulesInit.ai2do" target="_self">
	...
	<input type="hidden"  name="ciCustomGroupInfo.tacticsId" id="dimListTactics" value="<%=ServiceConstants.LIST_TABLE_TACTICS_ID_THREE %>"></input>
	...
	<input id="listCreateTime" type="hidden" name="ciCustomGroupInfo.listCreateTime" value="${ciCustomGroupInfo.listCreateTime}" />
...
<input type="hidden" value="true" class="J_is_calcucenter_to_createpage"/>
...
<div id="saveCustomerDialog" style="overflow:hidden; display:none;">
  <iframe id="saveCustomerFrame"  name="saveCustomerFrame"  src="" framespacing="0" border="0" frameborder="0" style="width:100%;height:569px;" scrolling="no"  ></iframe>
</div>
<div id="itemChoose" style="overflow:hidden;">
	<iframe id="itemChooseFrame" name="itemChooseFrame"  src="" framespacing="0" border="0" frameborder="0" style="width:100%;height:453px;" scrolling="no"  ></iframe>
</div>
<div id="darkValueSet" style="overflow:hidden;">
	<iframe id="darkValueSetFrame" name="darkValueSetFrame"  src="" framespacing="0" border="0" frameborder="0" style="width:100%;height:290px;" scrolling="no"  ></iframe>
</div>
...
<div id="successDialog" style="overflow:hidden;">
	<iframe id="successFrame" name="successFrame" src="" framespacing="0" border="0" frameborder="0" style="width:100%;height:430px;" scrolling="no"  ></iframe>
</div>
<div id="editDialog" style="overflow:hidden;">
	<iframe id="editFrame" name="editFrame" scrolling="no" allowtransparency="true" src="" framespacing="0" border="0" frameborder="0" style="width:100%;height:310px" ></iframe>
</div>

<div id="labelAttrValidateDialog" style="overflow:hidden;">
	<iframe id="labelAttrValidateFrame" name="labelAttrValidateFrame" scrolling="no" allowtransparency="true" src="" framespacing="0" border="0" frameborder="0" style="width:100%;height:300px"></iframe>
</div>
<!-- 推送对应标签 -->
