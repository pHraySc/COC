...
<head>
	...
	$(function(){
		...
		/* var attrVal = '${param["attrVal"]}';
		var attrName = '${param["attrName"]}';
		var calcuElement = '${param["calcuElement"]}';
		parent.$("._idClass").parent().parent().attr("attrName"); */
		
		var p = parent.$('._idClass').parent().parent()
		var attrVal = p.attr('attrVal');
		var attrName = p.attr('attrName');
		var calcuElement = p.attr('calcuElement');
	...
	function pagetable(){
		...
		var actionUrl = '${ctx}/ci/ciLabelInfoAction!findLabelDimValue.ai2do?'+$("#pageForm").serialize();
		$("#itemChooseListDiv").page( {url:actionUrl,param:para,objId:'itemChooseListDiv',callback:pagetable});
	}
	...
	function serchItemList(){
		...
		$.ajax({
			...
			success:function(html){
				$("#itemChooseListDiv").html("");
	            $("#itemChooseListDiv").html(html);
	...
	function setItems(){
		if(validateForm()){
			...
			parent.$("._idClass").parent().parent().attr("attrVal",itemKeyArr.join(","));
			parent.$("._idClass").parent().parent().attr("attrName",itemNameArr.join(","));
			...
			//$conditionMore.empty().width($conditionMore.parent().width()).append(p);
			$conditionMore.html('').append(p);
			$conditionMore.attr("title",title_temp_p);
			parent.submitRules();
			parent.iframeCloseDialog("#itemChoose");
		}
	}	
	</script>
</head>
        ...
        <div class="itemChooseBox fleft" id="itemChooseBoxPage">
            <div class="itemActionTopBox ">
                ...
                <div class="itemSearchBox fright">
                    <input type="text" class="fleft searchInput" id="dimName" onkeydown="if(event.keyCode == 13){serchItemList();}"/>
                    <input type="button" class="fleft searchBtn" onclick="serchItemList();"/>
                </div>
            </div>
            <jsp:include page="/aibi_ci/dialog/itemChooseList.jsp"></jsp:include>
        </div>
...
<!--             </a> -->
    ...
    <input type="hidden" name ="calcuElement" id="calcuElement"/>
</body>
</html>